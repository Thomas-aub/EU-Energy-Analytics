(function() {
    ...
    function calculateAndRenderStats(specificYear = null) {
        ...
        if(carbonIntensity < 50) d3.select("#stat-intensity").style("color", "#10b981");
        else if(carbonIntensity < 200) d3.select("#stat-intensity").style("color", "#f59e0b");
        else d3.select("#stat-intensity").style("color", "#ef4444");
    ...
    window.updateProductionChart = function(countryName) {
        d3.select("#countrySelect").property("value", countryName);
        render();
    };
